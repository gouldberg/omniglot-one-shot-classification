...
def classification_run(data_folder, folder, f_load, f_cost, ftype='cost'):
	assert ((ftype=='cost') | (ftype=='score'))

	# get file names
	with open(os.path.join(data_folder, folder, fname_label)) as f:
		content = f.read().splitlines()

	pairs = [line.split() for line in content]
	test_files  = [os.path.join(data_folder, pair[0]) for pair in pairs]
	train_files = [os.path.join(data_folder, pair[1]) for pair in pairs]
...
data_folder = '/media/kswada/MyFiles/dataset/omniglot/all_runs'


# -->
# run01 - run20
# each run:
#  - training:  1 image / class * 20 classes
#  - training:  1 image / class * 20 classes
...
# ----------
# where class labels are stored for each run
fname_label = 'class_labels.txt'

# for example:
# run01 class_labels.txt show
#  - run01/test/item01.png run01/training/class08.png  -->  test item01 matches to training class08 image
#  - run01/test/item02.png run01/training/class09.png  -->  test item02 matches to training class09 image
#  .. .
...
data_folder = '/media/kswada/MyFiles/dataset/omniglot/all_runs'
folder = os.path.join(data_folder, 'run' + rs)
fname_label = 'class_labels.txt'
...
# this is the class_labels in run01
print(len(content))
print(content[0])
...
# --------------------------------------------------------------------------------------------------------
# check LoadImgAsPoints(fn)
# --------------------------------------------------------------------------------------------------------

idx = 0

I = imread(train_files[idx])
I = np.array(I, dtype=bool)
I = np.logical_not(I)
